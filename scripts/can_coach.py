...
try:
    rospy.init_node('can_coach', anonymous=True)
    ...
    while not rospy.is_shutdown():
        ...
        velocity = gnewVel
        #add this back in when sound is working..
        #t = threading.Timer(0.5, printit)
        ...
        while True:
            ...
            if p != 0:
                ...
                if gnewLeadMeasurement != None:
                    leadPosition,relv,newVelocity,newAccel = lt.getClusteredRadar(p,writer = csvwriter_PANDA,recentLeadMeasurement = leadDist,velNaccel= True,gnewLeadMeasurement = gnewLeadMeasurement, gmyDetections = gmyDetections) #0.03 second timeout
                    gnewLeadMeasurement = None
